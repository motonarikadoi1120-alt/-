...
function placeBlock(type,pos){
  ...
  else if(type=='キャンプファイヤー') mat=matGold;
...
window.addEventListener('keydown',(e)=>{
  const speed=0.5;
  if(e.key==='w') player.pos.z-=speed;
  if(e.key==='s') player.pos.z+=speed;
  if(e.key==='a') player.pos.x-=speed;
  if(e.key==='d') player.pos.x+=speed;
  if(e.key==='e'){ // 商人と接触したら
    if(player.pos.distanceTo(merchant.mesh.position)<3) merchant.interact();
  }
  if(e.key==='1' && inventory['階段']>=1) placeBlock('階段',player.pos.clone().add(new THREE.Vector3(0,0,-2)));
  if(e.key==='2' && inventory['ドア']>=1) placeBlock('ドア',player.pos.clone().add(new THREE.Vector3(0,0,-2)));
  if(e.key==='3' && inventory['キャンプファイヤー']>=1) placeBlock('キャンプファイヤー',player.pos.clone().add(new THREE.Vector3(0,0,-2)));
});
</script>
</body>
</html>
